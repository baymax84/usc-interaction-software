	...
	namespace maestro
	{
		...
		class ServoController
		{
			public:
				...
				ServoController( const uint8_t n_channels, const std::string path = "",
				                 const bool connect = false );
				ServoController( const uint8_t n_devices,
				                 const uint8_t n_channels_each, const std::string path =
				                     "", const bool connect = false );
				ServoController( const std::vector<uint8_t> n_device_channels,
				                 const std::string path = "", const bool connect =
				                     false );
				ServoController( const std::string filename, const std::string path =
				    "", const bool connect = false );
				ServoController( std::ifstream &fin, const std::string path = "",
				                 const bool connect = false );
				ServoController( YAML::Parser &parser, const std::string path = "",
				                 const bool connect = false );
				ServoController( const YAML::Node &node, const std::string path = "",
				                 const bool connect = false );
				...
				bool
				isValidServoTarget( const uint8_t channel, const uint16_t target ) const;
				bool isValidServoTarget( const uint8_t device, const uint8_t channel,
				                         const uint16_t target ) const;
				double clipServoTargetValue( const uint8_t channel,
				                             const uint16_t target ) const;
				double clipServoTargetValue( const uint8_t device,
				...
				bool
				setServo( const uint8_t device, const uint8_t channel, Servo servo );
				bool setServoLimits( const uint8_t channel, ServoLimits limits );
				bool setServoLimits( const uint8_t device, const uint8_t channel,
				                     ServoLimits limits );
				bool setServoLimits( const uint8_t channel, uint16_t limit1,
				                     uint16_t limit2 );
				bool setServoLimits( const uint8_t device, const uint8_t channel,
				                     uint16_t limit1, uint16_t limit2 );
				bool setServoEnabled( const uint8_t channel, bool enabled );
				bool setServoEnabled( const uint8_t device, const uint8_t channel,
				                      bool enabled );
				bool setServoTarget( const uint8_t channel, uint16_t target );
				bool setServoTarget( const uint8_t device, uint8_t channel,
				                     uint16_t target );
				bool setServoSpeed( const uint8_t channel, uint16_t speed );
				bool setServoSpeed( const uint8_t device, const uint8_t channel,
				                    uint16_t speed );
				bool setServoAcceleration( const uint8_t channel, uint16_t accel );
				bool setServoAcceleration( const uint8_t device, const uint8_t channel,
				                           uint16_t accel );
				bool setServosHome();
				...
				ServoLimits
				getServoLimits( const uint8_t device, const uint8_t channel ) const;
				uint16_t getServoMinLimit( const uint8_t channel ) const;
				uint16_t
				getServoMinLimit( const uint8_t device, const uint8_t channel ) const;
				uint16_t getServoMaxLimit( const uint8_t channel ) const;
				uint16_t
				getServoMaxLimit( const uint8_t device, const uint8_t channel ) const;
				bool getServoEnabled( const uint8_t channel ) const;
				bool
				getServoEnabled( const uint8_t device, const uint8_t channel ) const;
				uint16_t getServoTarget( const uint8_t channel ) const;
				uint16_t
				getServoTarget( const uint8_t device, const uint8_t channel ) const;
				uint16_t getServoSpeed( const uint8_t channel ) const;
				uint16_t
				getServoSpeed( const uint8_t device, const uint8_t channel ) const;
				uint16_t getServoAcceleration( const uint8_t channel ) const;
				uint16_t getServoAcceleration( const uint8_t device,
...
void operator >>( const YAML::Node &nodes,
                  pololu::maestro::ServoController &servo_controller );
