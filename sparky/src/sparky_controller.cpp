...
//
SparkyController::SparkyController( const std::string device_path,
                                    const bool connect, const bool home ) :
	servo_controller_( N_DEVICES, N_CHANNELS_EACH, device_path )
{
	assert(initJoints());
	if ( connect ) SparkyController::connect( home );
...
//
SparkyController::JointLimits SparkyController::getJointLimits(
...
//
SparkyController::Joint::Joint( const uint8_t servo_device,
                                const uint8_t servo_channel,
