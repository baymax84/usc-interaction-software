...
int main( int argc, char** argv )
{
	...
	printf( "setServoLimits(%d, %d, <%d, %d>) = ", device, channel, limits.first,
	        limits.second );
	if ( servo_controller.setServoLimits( device, channel, limits ) ) printf(
	                                                                          "SUCCESS!!!\n" );
	else printf( "FAILURE...\n" );

	printf( "setServoEnabled(%d, %d, %d) = ", device, channel, enabled );
	if ( servo_controller.setServoEnabled( device, channel, enabled ) ) printf(
	                                                                            "SUCCESS!!!\n" );
	else printf( "FAILURE...\n" );

	printf( "setServoAcceleration(%d, %d, %d) = ", device, channel, accel );
	if ( servo_controller.setServoAcceleration( device, channel, accel ) ) printf(
	                                                                               "SUCCESS!!!\n" );
	else printf( "FAILURE...\n" );

	printf( "setServoSpeed(%d, %d, %d) = ", device, channel, speed );
	if ( servo_controller.setServoSpeed( device, channel, speed ) ) printf(
	                                                                        "SUCCESS!!!\n" );
	else printf( "FAILURE...\n" );

	printf( "setServoTarget(%d, %d, %d) = ", device, channel, target );
	if ( servo_controller.setServoTarget( device, channel, target ) ) printf(
	                                                                          "SUCCESS!!!\n" );
	else printf( "FAILURE...\n" );
