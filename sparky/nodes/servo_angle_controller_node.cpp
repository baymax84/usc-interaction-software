...
int main( int argc, char** argv )
{
	...
	printf( "setServoLimits(%d, %d, <%d, %d>) = ", device, channel, limits.first,
	        limits.second );
	if ( angle_controller.setServoLimits( device, channel, limits ) ) printf(
	                                                                          "SUCCESS!!!\n" );
	else printf( "FAILURE...\n" );

	printf( "setAngleLimits(%d, %d, <(%d, %.2f), (%d, %.2f)>) = ", device,
	        channel, min_limit.first, min_limit.second, max_limit.first,
	        max_limit.second );
	if ( angle_controller.setServoAngleLimits( device, channel, angle_limits ) ) printf(
	                                                                                     "SUCCESS!!!\n" );
	else printf( "FAILURE...\n" );

	printf( "setServoEnabled(%d, %d, %d) = ", device, channel, enabled );
	if ( angle_controller.setServoEnabled( device, channel, enabled ) ) printf(
	                                                                            "SUCCESS!!!\n" );
	else printf( "FAILURE...\n" );

	printf( "setServoAcceleration(%d, %d, %d) = ", device, channel, accel );
	if ( angle_controller.setServoAcceleration( device, channel, accel ) ) printf(
	                                                                               "SUCCESS!!!\n" );
	else printf( "FAILURE...\n" );

	printf( "setServoSpeed(%d, %d, %d) = ", device, channel, speed );
	if ( angle_controller.setServoSpeed( device, channel, speed ) ) printf(
	                                                                        "SUCCESS!!!\n" );
	else printf( "FAILURE...\n" );

	printf( "setAngleTarget(%d, %d, %.2f) = ", device, channel, target );
	if ( angle_controller.setServoAngleTarget( device, channel, target ) ) printf(
	                                                                               "SUCCESS!!!\n" );
	else printf( "FAILURE...\n" );
